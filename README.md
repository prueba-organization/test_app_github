[![Build Status](https://git.vauxoo.com/vauxoo/test_app_github/badges/11.0/build.svg)](https://git.vauxoo.com/vauxoo/test_app_github/pipelines)
[![Coverage Report](https://git.vauxoo.com/vauxoo/test_app_github/badges/11.0/coverage.svg)](https://git.vauxoo.com/vauxoo/test_app_github/commits/11.0)
[![Runbot Status](http://runbot.vauxoo.com/runbot/badge/109/11.0.svg)](http://runbot.vauxoo.com/runbot/repo/git-git-vauxoo-com-vauxoo-test_app_github-git-109)



Modules for test_app_github
===
...
1. Install travis2docker in order to a create local environment (docker
   container) copy of last changes on the test_app_github repository:
    ...
    of test_app_github for make functional tests or even for do new developments:
...
3. Now we can use the t2d command. This will be accomplish by using the next
   command line:
    ...
    ```bash
    $ t2d git@git.vauxoo.com:vauxoo/test_app_github.git 11.0
    ```

    The result after run this t2d script will be a log that have a list of
    local paths, in test_app_github case, will have a list of 2 paths

    ```bash
    ['/home/user/t2d/script/git_git.vauxoo.com_vauxoo_test_app_github.git/11.0/1',
    '/home/user/t2d/script/git_git.vauxoo.com_vauxoo_test_app_github.git/11.0/2']
    ```
...
4. The next step is to run the last mentioned scripts, we highly recommend to
   add the next extra arguments.
    ...
    ```bash
    /home/user/t2d/script/git_git.vauxoo.com_vauxoo_test_app_github.git/11.0/2/10-build.sh --no-cache
    /home/user/t2d/script/git_git.vauxoo.com_vauxoo_test_app_github.git/11.0/2/20-run.sh --entrypoint=bash
    ```
...
5. After the last script is finish you will notice that your prompt has change,
   ...
   like this:
    ...
    ```bash
    [root@a458896bf8f5]~/build/vauxoo/test_app_github$
    (11.0) $
    ```
    ...
    test_app_github:
        ...
        - main repository will be ALWAYS inside /root/build/vauxoo/test_app_github
